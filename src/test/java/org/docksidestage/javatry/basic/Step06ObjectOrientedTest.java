...
public class Step06ObjectOrientedTest extends PlainTestCase {
    ...
    public void test_objectOriented_zoo() {
        ...
        // _/_/_/_/_/_/_/_/_/_/
        // done jflute 次回1on1でふぉろー (2026/01/27)
        // #1on1: よく考えてくださってありがとうございます (2026/02/12)
        // 独自のプロパティを持つのがどうか？ってところで言うと、
        // サブクラスは独自の振る舞いを持つ限りは、独自のプロパティも持つ可能性は十分ある。
        //
        // 実装でフォーカスを当てるとすると...
        // o Animalをキープしつつ独自のものを追加するところではなく...
        // o Animalの特性を打ち消している(台無しにしている)ところ (hitPoint)
        // 実装面での判断材料で言うと、打ち消しがあるかどうか？
        // 打ち消しがあるってことは、Animalとして扱う、ということに無理が出てくる。
        // そう言う面でZombieは若干あやしいと言える。
        // ↑ここがボトムアップな考え方
        // 
        // ↓一方でトップダウンな考え方
        // is-aが成り立つかどうか？
        //
        // バイオハザードの世界で考えるZombieだと、すべての生き物がゾンビになりうる。
        // その場合、ゾンビというのは個体を指す名詞ではなく、状態を示す言葉とも言えないか!?
        // 単純実装だと: protected boolean isZombie;
        //
        // 一方で、ゾンビの世界のゾンビ王国、みたいな世界だったら、
        // それは普通にゾンビも種族の一つと言ってもいいかも。
        //
        // ということで答えがあるわけじゃないですが、
        // そのものの概念の捉え方によって、実装はガラリを変わる。
        // その合った実装ができないと、ひずみが生まれて、コードに無理が出てくる。
        // なので、正しく概念を捉える、というのは階層構造を作る上でとても重要。
        //
        // オブジェクト指向もでそれが大事。
        // 間違った階層構造で作ったプログラムは、逆に見づらい。
        // (それならベタ書きの方がマシなことも)
        // 概念力を鍛えよう。
        // 概念力の深掘り。
    }
