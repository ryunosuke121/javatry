...
public class Step07ExceptionTest extends PlainTestCase {
    ...
    // done jflute 次回1on1ふぉろーここから (2026/02/24)
    // ===================================================================================
    //                                                                               Cause
    //                                                                               =====
    /**
    ...
    // #1on1: 一つの落ちたというイベントにつき、複数の例外メッセージ(例外情報)がある (2026/03/11)
    // それぞれのレイヤーごとの抽象度の言い分がある。
    // 「例外の翻訳」
    // 例外が例外を持つことができる理由: causeを複数throwしたいから
    // throwできる例外は同時に1個だけ。
    // なので、cause技で例外をチェーンさせて実質的に複数throwしてる状態を作っている。
  ...
  /\           |     |        PK           ^^v
               ...
               +設定ファイル (PK)
             */
            // throwする側の人になったとき、将来の誰か(AIかも)がデバッグしやすい形で残す意識を付けて欲しい。
            // 一方で、本番トラブル対応する側になったとき、すべての例外メッセージをちゃんと読む意識を付けて欲しい。
            // (仕組みがわかれば自然と読むだろう)
            // // エラーメッセージ読め読め大合唱
            // https://jflute.hatenadiary.jp/entry/20130522/errorsinging
        }

        // #1on1: OSSのお話ガッツリ (2026/03/11)
        // OSSにコントリビュートしてるの素晴らしい。
    }
    ...
    public void test_exception_zone_differenceExceptionError() {
        ...
        //
        // _/_/_/_/_/_/_/_/_/_/
        // #1on1: 概念としてのException/Errorの言葉ではおおよそOK (2026/02/12)
        // JavaのExceptionクラス/Errorクラスってところで言うと、ちょっと補足。
        // 
        // Errorクラスの例: throwする瞬間にシステムエラーと言い切れるような状態。
        // Exceptionクラスの例: ↑の状態もあるけど...業務例外やリカバリ可能例外もある。
        // 基本的にはthrowした瞬間はシステムエラーなのか正常なレアケースなのかまだわからない。
        // catchした人が初めて判断できる。
        // (throwされた例外がエラーと判断されたり例外のままリカバリされたり)
        //
        // 言語ごとの話:
        // Goだと、JavaのExceptionがErrorで、JavaのErrorがPanicみたいな by いとりゅうさん
        // TypeScriptの例: 実装はErrorしかないけど、ドキュメントでは例外処理。
        //
        // 処理を止めるきっかけを最初に検知した人: throwする人、戻り値でerrorを戻す人
        // その検知された「処理を止めた情報」を受け取った人: catchする人、戻り値のerrorを受け取った人
    }
    /*
                           +-----------------+
                           |                 |
